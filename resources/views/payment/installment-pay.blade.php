...
<div class="card">
    <div class="card-header">
        ...
        <h1>ชำระค่างวดที่ {{ $installment->installment_no }} / {{ $booking->installment_count }}</h1>
    ...
    <div class="card-body">
        ...
        @if (session('paid_installment_no'))
            <div class="alert alert-success">
                ...
                ทีมงานกำลังตรวจสอบ ระบบจะส่งอีเมลยืนยันให้ครับ
            </div>
        @endif
        ...
        @if ($isOverdue)
            <div class="alert alert-error">
                ⚠️ งวดนี้เลยกำหนดชำระแล้ว ({{ $dueDate }}) กรุณาชำระโดยเร็วเพื่อรักษาสิทธิ์การเดินทาง
            </div>
        @endif

        <div class="amount-box">
            <div class="amount-label">ยอดที่ต้องชำระ — งวดที่ {{ $installment->installment_no }}</div>
        ...
        <div class="info-list">
            <div class="info-row copy-row">
                ...
                <span class="value">
                    <span id="ppId">{{ config('payment.promptpay_id_display') }}</span>
            ...
            <div class="info-row">
                <span class="label">ชำระแล้วรวม</span>
        ...
        <form method="POST" action="{{ route('public.pay.submit', $booking->payment_token) }}" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn">ยืนยันการชำระ งวดที่ {{ $installment->installment_no }}</button>
        </form>
