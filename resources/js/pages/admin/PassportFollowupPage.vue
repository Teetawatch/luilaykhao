<template>
  <div class="admin-page">
    <div class="page-header">
      <div>
        <h1 class="page-title">
          <span class="material-symbols-rounded heading-icon">badge</span>
          ตามเก็บพาสปอร์ต
        </h1>
        <p class="page-subtitle">
          การจองทริปต่างประเทศที่ยังไม่มีข้อมูลหน้าพาสปอร์ตครบทุกคน — ส่วนใหญ่มาจากแอปรุ่นเก่าที่ยังไม่มีช่องกรอก
          กดคัดลอกลิงก์แล้วส่งให้ลูกค้า (คนจอง) กรอกให้ผู้เดินทางทุกคนได้ในหน้าเดียว
        </p>
      </div>
    ...
    <div class="filter-bar">
      ...
      <div class="summary-pill">
        ...
        ค้างอยู่ {{ filtered.length }} การจอง · ขาดพาสปอร์ตรวม {{ totalMissing }} คน
      </div>
    </div>
    ...
    <div class="table-card">
      ...
      <div v-else>
        <div v-if="!filtered.length" class="empty-state">
          ...
          ไม่มีการจองที่ค้างพาสปอร์ต — ครบทุกคนแล้ว 🎉
        </div>
        ...
        <table v-else class="data-table">
          <thead>
            <tr>
              ...
              <th>คนจอง</th>
              <th>ยังขาด</th>
              <th style="text-align:right;">ลิงก์กรอกพาสปอร์ต</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in filtered" :key="row.booking_id">
              <td>
                ...
                <span v-if="row.country_label" class="country-tag">{{ row.country_label }}</span>
                <div class="cell-sub">
              ...
              <td>
                ...
                <div class="cell-sub">{{ (row.missing_names || []).join(', ') }}</div>
              </td>
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await api.get('/admin/passport-followup');
