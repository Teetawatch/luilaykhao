...
class TripSchedule extends Model
{
    ...
    /**
     * บรรทัด "ขึ้นรถวันไหน" ของรอบที่ออกก่อนวันทริป — null เมื่อออกวันเดียวกับวันทริป
     *
     * เช่น "ขึ้นรถ ศุกร์ที่ 4 กันยายน 2569 เวลา 20:00 น. — รถออกก่อนวันทริป 1 วัน"
     * เขียนวันให้เต็มพร้อมชื่อวัน ไม่ใช่แค่เวลา — คนที่มาผิดวันคือคนที่อ่านเจอแค่
     * "20:00 น." แล้วเติมวันที่ของทริปให้เอง ส่งไปทั้ง API ให้หน้าจอที่ไม่มี
     * ตัวช่วยวันที่ไทย (LIFF) พิมพ์ตามได้เลย ไม่ต้องคิดวันเอง
     */
    public function earlyDepartureLabelThai(): ?string
    {
        if (! $this->departsBeforeTripDay()) {
            return null;
        }

        $departsAt = $this->departs_at;

        return 'ขึ้นรถ '.$departsAt->locale('th')->isoFormat('dddd').'ที่ '.ThaiDate::full($departsAt)
            .' เวลา '.$departsAt->format('H:i').' น. — รถออกก่อนวันทริป '.$this->daysDepartingEarly().' วัน';
    }
    ...
    /**
     * รอบนี้ยังรับคนใหม่ได้ไหม — ใช้กับลิงก์เก็บข้อมูลลูกค้าที่แปะค้างไว้ในไบโอ
     * หรือ auto-reply ไลน์ ลิงก์เดิมถูกเปิดต่อไปเรื่อย ๆ หลังรอบเต็มหรือออกไปแล้ว
     *
     * นับ bookable_seats จึงไม่กินที่ที่กันไว้ให้คนในคิวรอที่ได้รับสิทธิ์อยู่
    ...
    public function acceptsNewCustomers(bool $joinTrip = false): bool
    {
        ...
        // เทียบด้วยวัน ไม่ใช่ departs_at — departs_at เก็บเวลาไทยในคอลัมน์ชนิด UTC
        // การเทียบตรง ๆ จะเพี้ยนไป 7 ชั่วโมง
        if ($this->departure_date && $this->departure_date->toDateString() < now('Asia/Bangkok')->toDateString()) {
            return false;
        }
