...
class CustomerIntake extends Model
{
    ...
    /**
     * คืนกลุ่มนี้กลับไปอยู่ในหมวด "ยังไม่ได้จอง" ให้ดึงไปเปิดการจองใหม่ได้
     *
     * "ดึงไปจอง" ไม่ได้แปลว่าจองสำเร็จ — ใบจองที่เปิดไว้เป็น pending แล้วลูกค้า
     * โอนไม่ทัน จะถูกยกเลิกอัตโนมัติภายในไม่กี่นาที ถ้ากลุ่มยังค้างเป็น "จองแล้ว"
     * อยู่ ข้อมูลที่ลูกค้ากรอกมาจะกลายเป็นข้อมูลตายที่ทีมงานหยิบมาใช้ไม่ได้อีกเลย
     *
     * `booking_id` ยังชี้ใบจองที่พังไว้เหมือนเดิม เป็นร่องรอยว่าเคยดึงไปแล้วครั้งหนึ่ง
     * ส่วน `converted_at` ต้องว่าง เพราะการดึงครั้งนั้นไม่ได้กลายเป็นการจองจริง
     */
    public function reopen(): bool
    {
        ...
        $this->forceFill([
            'status' => 'new',
            'converted_at' => null,
            // เริ่มนับอายุการเก็บใหม่จากวันนี้ ไม่งั้นกลุ่มที่เพิ่งกลับมารออาจโดน
            // งานลบอัตโนมัติกวาดทิ้งทันที เพราะความเคลื่อนไหวล่าสุดคือตอนลูกค้ากรอก
    ...
    /** เคยถูกดึงไปเปิดการจองแล้วแต่ใบนั้นไม่สำเร็จ — ยังรอทีมงานหยิบไปจองใหม่ */
    public function wasReopened(): bool
    {
        return $this->status === 'new' && $this->booking_id !== null;
    }
