...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | นโยบายยกเลิก — ทริปต่างประเทศ
    |--------------------------------------------------------------------------
    |
    | คนละชุดกับทริปในประเทศโดยตั้งใจ เพราะโครงสร้างต้นทุนไม่เหมือนกันเลย:
    | ทริปในประเทศเราจ่ายค่ารถ/ที่พักทีหลังและเลื่อนได้ แต่ทริปต่างประเทศต้อง
    | ออกตั๋วเครื่องบินเป็นชื่อผู้เดินทางล่วงหน้า ซึ่งสายการบินไม่คืนเงินและ
    | เปลี่ยนชื่อไม่ได้ ถ้าใช้นโยบายเดียวกัน "คืนเต็มจำนวนก่อน 45 วัน" จะกลายเป็น
    | คำสัญญาที่เราจ่ายเองทั้งก้อน
    |
    | ตัวเลขวันและเปอร์เซ็นต์ผูกกับเงื่อนไขตั๋วของแต่ละปลายทาง ทีมงานปรับได้ที่
    | .env โดยไม่ต้องแก้โค้ด
    |
    */
    'cancellation_policy_international' => [
        'free_change_days' => (int) env('CANCELLATION_INTL_FREE_CHANGE_DAYS', 60),
        // เส้นแบ่งของขั้นกลาง (คืนบางส่วน) — โค้ดคำนวณยอดคืนอ่านสองค่านี้ ไม่ได้
        // ฮาร์ดโค้ดไว้อีกที่ ตัวเลขที่ลูกค้าเห็นกับตัวเลขที่ระบบคืนจึงตรงกันเสมอ
        'partial_refund_days' => (int) env('CANCELLATION_INTL_PARTIAL_REFUND_DAYS', 45),
        'partial_refund_percent' => (int) env('CANCELLATION_INTL_PARTIAL_REFUND_PERCENT', 50),
        'tiers' => [
            [
                'range' => 'ก่อนออกตั๋วเครื่องบิน (มากกว่า 60 วัน)',
                'detail' => 'คืนเงินเต็มจำนวน หรือเลื่อนรอบ / เปลี่ยนผู้เดินทางแทนได้ฟรี',
                'percent' => 100,
            ],
            [
                'range' => 'หลังออกตั๋วแล้ว 45–60 วันก่อนเดินทาง',
                'detail' => 'คืนเงินส่วนที่ไม่ใช่ค่าตั๋วเครื่องบินและค่ามัดจำที่ปลายทางจ่ายไปแล้ว',
                'percent' => 50,
            ],
            [
                'range' => 'ก่อนเดินทางน้อยกว่า 45 วัน',
                'detail' => 'ขอสงวนสิทธิ์ไม่คืนเงิน ตั๋วเครื่องบินและค่าธรรมเนียมวีซ่าคืนไม่ได้ '
                    .'และเปลี่ยนชื่อผู้เดินทางบนตั๋วไม่ได้',
                'percent' => 0,
            ],
        ],
        'note' => 'ค่าตั๋วเครื่องบิน ค่าธรรมเนียมวีซ่า และค่าใบอนุญาตขึ้นเขา เป็นค่าใช้จ่ายที่ '
            .'ผู้ให้บริการปลายทางไม่คืนให้เราเช่นกัน จึงไม่รวมอยู่ในยอดที่คืนได้ '
            .'แนะนำให้ทำประกันการเดินทางที่คุ้มครองการยกเลิก หากผู้จัดยกเลิกทริปเอง '
            .'ลูกค้าจะได้รับเงินคืนเต็มจำนวนทุกกรณี',
    ],
];
